 ...
 *  作用:遍历数组或者遍历对象
 *  语法:
 *    1)  遍历数组   v-for="(数组每一项,索引) in 数组" :key="唯一标识" 
 *    2)  遍历数组   v-for="数组每一项 in 数组"  :key="唯一标识"
 *    3)  遍历对象   v-for="(值,键,索引) in 对象" :key="唯一标识"
 * 
 * 
 * ref 是一个属性(在标签上使用的属性),在DOM树中的作用是用来获取某个DOM元素的,但是需要通过$refs对象来获取,是单独的使用的
 * 
 * 
 * 计算属性:当某个属性的值发生变化,相关联的属性的值会自动的发生变化
 * computed
 * 
 * 监视:某个数据发生变化,需要做一些操作
 * watch
 * 
 * 实例化Vue对象的时候,配置对象中可以写哪些内容:
 * el--->选择器
 * data--->数据对象
 * methods--->对象 回调
 * computed--->对象 计算属性的
