...
自动登录三种方式:
1.localStorage:(用户登录成功后,把用户的相关信息存储在浏览器的缓存中,刷新页面后,从缓存中读取数据,实现自动登录)
               手动清理了浏览器的缓存,那么自动登录就失效了,一般会自动的跳转到登录界面
2.cookie:(登录成功后,后台会创建一个持久化的cookie,嵌入用户的相关信息,返回给浏览器,刷新页面后,从本地的cookie中读取用户的相关信息,发送给服务器,服务器要进行验证,cookie有一个有效期,验证该有效期是否已经失效,如果失效了,先告诉浏览器无法实现自动登录,会重新的进行登录)
   cookie: 会话cookie(内存中)和持久化cookie(本地,服务器创建的,maxAge需要设置)
3.token:加密的字符串,登录成功后,服务器端会创建token对象信息,由(密钥,有效期,用户的信息)组合起来,创建一个token,产生token的加密字符串,返回给浏览器,浏览器这边验证是否有token,如果有token,那就证明,已经登录成功了,从而实现自动登录的操作,token实际上会存储在浏览器端,可以存储到vuex中,也可以存储到localStorage中,刷新页面,从vuex或者从localStorage中读取出token字符串信息,放在请求头中进行携带,发送给服务器,服务器端验证token,解密token字符串,验证是否超过了有效期,如果超过了,肯定需要重新登录,否则实现自动的登录
token的作用:不仅可以实现自动登录,同时也可以实现访问特定接口需要有token的操作
我的订单信息(调用api接口,展示订单页面),如果没有登录,就没有token,那么此时调用接口,在请求头没有携带token,抱歉,无法调用这个接口,没有办法,查看订单界面
 服务器端解密token后,如何验证携带的用户信息是正确的从而实现自动登录,服务器端解密token后会把内部的用户的相关信息取出来(用户id),和数据库中的用户的id进行对比,如果有(匹配了),那么说明用户是存在,从而最终实现自动登录

localStorage:浏览器端
cookie:持久化的cookie---->服务器端创建的---->是有时间限制的(有效期)---->浏览器打开会自动的携带(服务器验证有效期)
cookie:临时cookie----->浏览器端创建的------>浏览器关闭,就不存在了
token:服务器端--->加密--->有期限的(有效期)
   应用:一般访问特定接口需要携带token,实现自动登录

 例子: 访问购物车界面,没有登录,请求头中携带的userTempId(临时用户id凭证),可以查看购物车的数据--->查看的是临时用户的购物数据
                     登录了,请求头中携带token(用户已经登录),查看购物车中的数据----->查看的是用户自己的购物数据

   用户A先登录(token),向购物车中添加了两件商品:笔记本,茶杯
   退出了
   用户A:没有登录(userTempId),向购物车中添加一件商品:手机
   用户再次登录(token,userTempId),此时购物车中的商品: 笔记本,茶杯,手机  原因:后台根据临时用户id把数据拿出来再把登录后的数据也拿出来,进行合并
...
31. 
   ...
   路由守卫(导航守卫)
   路由:就是对象,是一种映射关系,指的是地址和组件之间的一种映射关系,通过可以使用路由链接和路由视图形成这种映射关系,前提是需要先进行地址和组件的关系的注册
   路由守卫:监视路由及控制路由跳转的一种技术,守卫(保护)路由的
   路由守卫的应用:监视一些路由,对路由的跳转进行深度的控制(强制路由的跳转),例子:监视一个路由,在跳转的时候,进行跳转的控制,可以作为权限的操作,如果没有登录,想要跳转到结算界面,此时可以使用路由守卫,监视要跳转的地址,强制的跳转到登录的界面

   路由守卫(导航守卫):
    1.全局路由守卫
      1) 全局前置守卫:在路由跳转之前发生
      2) 全局解析守卫:路由解析过程中发生(全局的前置守卫执行后,组件内守卫执行后，后置守卫执行前发生)
      3) 全局后置守卫:路由跳转结束后发生
    2.路由独享守卫
      前置守卫
      注册路由的时候,进行路由的监视及控制
    ...
    3.组件内守卫
      组件内部涉及到路由跳转的时候,进行路由的监视及控制
32.使用qrcode 插件,生成二维码的图片地址
33.使用element-ui组件库 设置对话框及提示消息的展示

34. 路由的懒加载:按需引入
    引入所有的路由组件,项目之后,所有的引入都已经打包了,但是页面打开后,未必就会用到这个组件界面,此时的打包,把最终的打包文件变的很臃肿,所以,可以使用路由懒加载的方式,来进行优化操作,实现按需引入
35. 图片的懒加载  lazyload插件
