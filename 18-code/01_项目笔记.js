...
25.浏览器缓存的方式(面试题):
localStorage: 存储在浏览器的本地----->浏览器关闭,重新打开--->数据依然存在----->相对来讲速度有一点点慢
sessionStorage: 存储在浏览器的内存---->浏览器关闭,重新打开--->数据被清除----->相对来讲速度有一点快
使用方式都一样: setItem(key,值)设置数据  getItem(键)获取数据  removeItem(键)移除数据
...
27. action内部是可以调用其他的action的



28. 为什么直接使用地址获取验证码和通过代理方式获取验证码注册的时候是不同的结果? 
  1)页面中直接使用地址的方式向服务器发送请求获取图形验证码(http引擎和服务器进行交互) 
   注册的是时候使用的是代理的方式和服务器进行交互,两次交互的方式是不一样的,所以,验证码注册输入的时候是失败的
  2) 页面中使用代理的方式和服务器进行交互,获取图形验证码(ajax引擎)
     注册时候使用的也是代码的方式和服务器进行交互(ajax引擎),此时,两次请求的方式都是一样,注册的时候携带的验证码是没有问题的,注册是可以成功的

    发送了请求,服务器内部是需要把生成的验证码进行保存的,注册的时候发送的图形码和服务器的图形码会进行对比(请求方式)



29.正则:有规则,(神经学科)正则表达式:又叫规则表达式,有一些符号组成的式子,可以进行字符串(数字,特殊符号)的匹配操作
邮箱的正则:
用户名@域名
字母   数字  _  -  .   ----用户名
@
...
正则的作用:
匹配字符串, 做表单的验证,可以爬虫,例子: 爬取整个页面中的所有的图片,整个页面中的所有的视频(电影)
